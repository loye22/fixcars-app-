...
class _TapiterieScreenState extends State<TapiterieScreen> {
  ...
  final MecanicAutoService _tapiterieService = MecanicAutoService();
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> services = await _tapiterieService.fetchMecanicAutos(
        category: AutoService.tapiterie_auto,
        lat: _currentLat,
        lng: _currentLng,
  ...
  Widget build(BuildContext context) {
    if (services.isEmpty) {
      return Container(
        ...
        child: const Column(
          children: [
            Icon(Icons.airline_seat_recline_extra_outlined, color: Color(0xFF141414), size: 80),
            SizedBox(height: 16),
            Text("Niciun atelier de tapițerie găsit în zonă.", style: TextStyle(color: Color(0xFFAAAAAA))),
          ],
        ),
      );
    }
