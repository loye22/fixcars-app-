...
class _VulcanizareScreenState extends State<VulcanizareScreen> {
  ...
  Map<String, dynamic>? _selectedBrand; // Logic preserved
  ...
  final MecanicAutoService _vulcanizareService = MecanicAutoService();
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> services = await _vulcanizareService.fetchMecanicAutos(
        category: AutoService.vulcanizare_auto_mobila,
        lat: _currentLat,
        lng: _currentLng,
  ...
  Widget _buildMechanicList() {
    if (_mechanicServices.isEmpty) {
      return Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.tire_repair_outlined, color: _darkCard, size: 80),
            const SizedBox(height: 16),
            const Text("Nicio vulcanizare găsită în zonă.", style: TextStyle(color: _secondaryText)),
          ],
        ),
      );
    }
      ...
      children: _mechanicServices.map((service) {
        ...
        return Container(
          ...
          child: BusinessCardWidget(
            ...
            businessName: service['supplier_name'] ?? 'Vulcanizare Auto',
