...
class CaroserieSiVopsitorieScreen extends StatefulWidget {
  @override
  _CaroserieSiVopsitorieScreenState createState() =>
      _CaroserieSiVopsitorieScreenState();
}

class _CaroserieSiVopsitorieScreenState
    extends State<CaroserieSiVopsitorieScreen> {
  ...
  // Location
  double _currentLat = 45.6486; // Romania centre
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      final List<Map<String, dynamic>> data = await _service.fetchMecanicAutos(
        category: AutoService.caroserie_si_vopsitorie,
        lat: _currentLat,
        lng: _currentLng,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLocationLoading || _isLoading
        ...
        children: [
          _buildHeader("Tip Serviciu"),
          ServiceSelectionWidget(
            key: const ValueKey('service_selector_caroserie'),
            onServicesSelected: _onServicesSelected,
            initialSelectedServices: _selectedServices,
            AutoServicetype: 'caroserie_si_vopsitorie',
          ),
          const SizedBox(height: 24),
          _buildHeader("Ateliere Caroserie"),
          _buildMechanicList(),
        ],
      ),
    );
  ...
  Widget _buildMechanicList() {
    if (_services.isEmpty) {
      return Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.format_paint_outlined, color: _darkCard, size: 80),
            const SizedBox(height: 16),
            const Text("Niciun atelier de vopsitorie găsit.", style: TextStyle(color: _secondaryText)),
          ],
        ),
      );
    }
