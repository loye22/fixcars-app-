...
class _ITPScreenState extends State<ITPScreen> {
  ...
  final MecanicAutoService _itpService = MecanicAutoService();
  ...
  Future<void> _fetchData() async {
    ...
    try {
      ...
      List<Map<String, dynamic>> services = await _itpService.fetchMecanicAutos(
        category: AutoService.itp,
        lat: _currentLat,
        lng: _currentLng,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLocationLoading || _isLoading
        ...
        children: [
          _buildHeader("Tip Serviciu ITP"),
          ServiceSelectionWidget(
            key: const ValueKey('service_selector_itp'),
            onServicesSelected: _onServicesSelected,
            initialSelectedServices: _selectedServices,
            AutoServicetype: 'itp',
          ),
          const SizedBox(height: 24),
          _buildHeader("Stații ITP Disponibile"),
          _buildMechanicList(),
        ],
      ),
    );
  ...
  Widget _buildMechanicList() {
    if (_mechanicServices.isEmpty) {
      return Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.fact_check_outlined, color: _darkCard, size: 80),
            const SizedBox(height: 16),
            const Text("Nicio stație ITP găsită.", style: TextStyle(color: _secondaryText)),
          ],
        ),
      );
    }
