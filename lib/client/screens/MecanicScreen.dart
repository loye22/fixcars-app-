...
class _MecanicScreenState extends State<MecanicScreen> {
  ...
  Future<void> _fetchData() async {
    ...
    try {
      setState(() { _isFetchingData = true; _isLoading = true; _error = null; });
      List<Map<String, dynamic>> services = await _mechanicService.fetchMecanicAutos(
        category: AutoService.mecanic_auto,
        lat: _currentLat,
        lng: _currentLng,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: _isLoading
        ...
        children: [
          _buildHeader("Brand"),
          BrandSelectorWidget(onBrandSelected: (brand) {
            setState(() => _selectedBrand = brand);
            _fetchData();
          }),
          const SizedBox(height: 24),
          _buildHeader("Servicii disponibile"),
  ...
  Widget _buildMechanicList() {
    if (_mechanicServices.isEmpty) {
      return Container(
        ...
        child: Column(
          children: [
            const Icon(Icons.search_off_outlined, color: _darkCard, size: 80),
            const SizedBox(height: 16),
            const Text("Niciun mecanic găsit în această zonă.", style: TextStyle(color: _secondaryText)),
          ],
        ),
      );
    }
      ...
      children: _mechanicServices.map((service) {
        return Container(
          ...
          child: BusinessCardWidget(
            ...
            businessName: service['supplier_name'] ?? 'Mecanic Auto',
    ...
    showModalBottomSheet(
      ...
      builder: (context) => _LocationPickerModal(
        ...
        onLocationSelected: (lat, lng, addr) {
          setState(() { _currentLat = lat; _currentLng = lng; _address = addr; });
          _fetchData();
        },
      ),
    );
...
class _LocationPickerModalState extends State<_LocationPickerModal> {
  ...
  // Use constant colors locally to ensure consistency
  static const Color _darkBackground = Color(0xFF0A0A0A);
  static const Color _navBarColor = Color(0xFF1A1A1A);
  static const Color _accentSilver = Color(0xFFB0B0B0);
