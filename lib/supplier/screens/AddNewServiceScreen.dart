...
class _AddNewServiceScreenState extends State<AddNewServiceScreen> {
  ...
  final Map<String, Set<String>> _servicesByBrand = {};

  final AddNewService _addService = AddNewService();
  final ScrollController _scrollController = ScrollController();
  String? _brandWithNoServices;
  ...
  String? _selectedCityValue = 'Bucharest';
  String _selectedSectorValue = 'sector_1';
  ...
  static const _accentCyan = Color(0xFF00F0FF);
  static const _accentGreen = Color(0xFF00FF9D);
  static const _accentWarning = Color(0xFFFFB800);
  static const _accentDanger = Color(0xFFFF4B4B);
  static const _mutedText = Color(0xFF808080);
  ...
  void _nextStep() {
    if (_currentStep == 0) {
      if (_selectedBrandIds.isEmpty && !_selectAllBrands) {
        _showSnackBar('SELECTAȚI CEL PUȚIN O MARCĂ');
        return;
      }
    } else if (_currentStep == 1) {
      // Re-implementing the original logic for "Select All" vs Individual
      if (_selectAllBrands) {
      ...
      setState(() => _brandWithNoServices = null);
    }
  ...
  void _submit() async {
    ...
    for (final brandId in _selectedBrandIds) {
      ...
      if (services.isNotEmpty) {
        payloads.add({"brand_id": brandId, "service_ids": services.toList()});
      }
    }
    ...
    setState(() {
      ...
      if (_selectAllBrands) {
        for (final brand in brands) {
          final id = brand['brand_id'] as String;
  ...
  Widget _buildStepIndicator() {
    // final labels = ['BRANDS', 'SERVICES', 'FINALIZE'];
    final labels = ['BRANDURI', 'SERVICII', 'FINALIZEAZĂ'];
  ...
  Widget _buildStep1Brands() {
    ...
    return ListView(
      ...
      children: [
        ...
        _buildTitle('SELECTEAZĂ VEHICUL'),
        const SizedBox(height: 25),
        _buildAllBrandsToggle(),
        const SizedBox(height: 25),
        GridView.builder(
          shrinkWrap: true,
          physics: const NeverScrollableScrollPhysics(),
  ...
  Widget _buildAllBrandsToggle() {
    return Container(
      ...
      child: CheckboxListTile(
        ...
        onChanged: (v) => _toggleAllBrands(),
      ),
    );
  }

  Widget _buildStep2Services() {
    // Correctly handling "Select All Brands" logic from original code
    if (_selectAllBrands) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        final brand = brands[index];
        final id = brand['brand_id'];
        final services = _servicesByBrand[id] ?? {};
        final isProblem = _brandWithNoServices == id;
  ...
  Widget _buildAllBrandsServicesList() {
    final List<dynamic> categories = _data!['services_by_category'];
    return ListView(
      padding: const EdgeInsets.symmetric(horizontal: 24),
      children: [
        Container(
        ...
        ...categories.map((cat) => _buildCategoryTile(cat, null)).toList(),
      ],
    );
  }

  List<Widget> _buildServiceCategories(String brandId) {
    final List<dynamic> categories = _data!['services_by_category'];
    return categories.map((cat) => _buildCategoryTile(cat, brandId)).toList();
  }
  ...
  Widget _buildStepSummary() {
    int totalServices = 0;
    for (var s in _servicesByBrand.values) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          const SizedBox(height: 20),
          _buildTitle('REZUMATUL MISIUNII'),
